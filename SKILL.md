...
|---|---|---|---|---|---|
| **概念** | 构思、概念、方向、想法、灵感、几个版本、帮我想 | S1（轻量登记）→ S3a；模糊级需求先在 D0 后**停一次让用户选处境**，再搜；交付 3 候选后**再停** | `concept-generation.md`；`stage-2a-concept.md`；`director-lenses.md` 只看末尾速查表 | **无**（选定后才写 `02_script/concept.md`） | 对话里：D0 处境行 + D1 领域行 + 3 个同等发展的候选 + 推荐 + 一个问题；不预选；等用户选 |
...
停靠点规则：
- **S3a 后必停**（除非全流程）：概念未选定前**不创建任何文件**，包括 concept.md、bible、场景卡。概念是用户的决定，文件是承诺。
- **S5 后必停**（除非全流程）：分镜卡是最后一个人类可读的决策层，Prompt 一旦编译就只剩可观察量。
- 停下时只问一个问题：选哪个 / 是否按此继续。候选选择不超过 3 个选项；处境选择可多选、6–8 项。
...
12. **概念必须走五步协议**（`references/concept-generation.md`：禁用 → 种子 → 情境 → 评分 → 发展）。种子由模型用四个引擎（制度移植 / 骰子自转 / 物件履历 / 检索核实）自己生产，**永远不因为缺种子而停下**；先做 D0 内核展开（≥ 6 个不同处境，默认处境最多占一条查询；模糊级需求把处境交给用户选后再搜），有 WebSearch 时再做 D1 题材发现（跨处境、跨形式）与 D2 规则核实，事件级禁搜，一轮 ≤ 5 次；三个候选各做 D2、不预选；选定前不落盘；示例（在 `examples/`，概念模式不读）的查询与候选禁止复用；选定落盘后跑 `scripts/validate_concept.py`；无 WebSearch 则标 `[未核实]`；用户的真实细节是加分项，交付时附一句邀请即可。
