...
12. **概念必须走五步协议**（`references/concept-generation.md`：禁用 → 种子 → 情境 → 评分 → 发展）。种子由模型用四个引擎（制度移植 / 骰子自转 / 物件履历 / 检索核实）自己生产，**永远不因为缺种子而停下**；有 WebSearch 时必须核实候选依赖的规则（≤ 4 次搜索，禁搜"创意 / 点子"），无则标 `[未核实]`；用户的真实细节是加分项，交付时附一句邀请即可。
