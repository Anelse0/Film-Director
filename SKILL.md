...
|---|---|---|---|---|---|
| **概念** | 构思、概念、方向、想法、灵感、几个版本、帮我想 | S1（轻量登记）→ S3a，**停**（无种子时先停一次问三句；想不出来则给采集清单或抽骰子） | `concept-generation.md`；`stage-2a-concept.md`；`director-lenses.md` 只看末尾速查表 | `02_script/concept.md` | 对话里给 3 个候选 + 推荐 + 每个候选"目标格式下拍什么"一行；等用户选 |
...
12. **概念必须走五步协议**（`references/concept-generation.md`：禁用 → 种子 → 情境 → 评分 → 发展）。没有用户种子时先问三句；用户想不出来时按 §2b 走采集 → 骰子 → 借，模型任何情况下不自己编种子。
